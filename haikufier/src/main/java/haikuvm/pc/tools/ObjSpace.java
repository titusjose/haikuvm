...
extern statics_t allStatics;
	...
	public static void printH(PrintOnChange haikuConfigh) {
		...
		haikuConfigh.printf("#define STATIC_SIZE (STATIC_ROOTS*sizeof(%s)", list[1]);		
		for (int i = 2; i < list.length; i+=2) {
		    int count=count(list[i]);
			if (count>0) haikuConfigh.printf(" + %d*sizeof(%s)", count, list[i+1]);
		}
		haikuConfigh.printf(")\n");

		haikuConfigh.printf("#if defined(HAIKU_MemorySize)\n");
		haikuConfigh.printf("#define HEAPSIZE (HAIKU_MemorySize - STATIC_SIZE)\n");
		haikuConfigh.printf("#else\n");
		haikuConfigh.printf("#if defined(ARDUINO_SAM_DUE)\n");
		haikuConfigh.printf("#define HEAPSIZE (10000 - STATIC_SIZE)\n");
		haikuConfigh.printf("#else\n");
        //haikuConfigh.printf("#define HEAPSIZE (((RAMEND-0x100) - ((HAIKU_Mode==HAIKU_32_64||HAIKU_Mode==HAIKU_16_64)?2:1)* 500) - STATIC_SIZE)\n");
        haikuConfigh.printf("#define HEAPSIZE (((RAMEND-0x100) - 500) - STATIC_SIZE)\n");
		haikuConfigh.printf("#endif\n");
		haikuConfigh.printf("#endif\n");
		haikuConfigh.printf("\n");
		haikuConfigh.printf("typedef struct {\n");
		haikuConfigh.printf("	heap_t heapControl;\n");
		...
		haikuConfigh.printf("\n");
		haikuConfigh.printf("extern statics_t allStatics;\n");
//        outc.printf("\n");
...
//            for (VarInfo varInfo : map.values()) {
//                if (varInfo.type.equals(list[i]) && varInfo.natives!=0) {
//                    outc.printf("extern volatile %-10s %s; //gets=%d puts=%d native=%d\n", list[i+1], nativeC(varInfo.name), varInfo.gets, varInfo.puts, varInfo.natives);
//                }
//            }
//        }
//        outc.printf("#endif\n");;
