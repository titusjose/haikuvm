...
import static haiku.avr.lib.arduino.ArduinoLib.*;
...
public abstract class Print {
    ...
    private static int write_error=0;
	   ...
	   protected void setWriteError(int err) { write_error = err; };
	   
	   protected void setWriteError() { setWriteError(1); }
	    public int getWriteError() { return write_error; }
	    public void clearWriteError() { setWriteError(0); }

		public void begin(long speed) {
			short t = (short) (((F_CPU >> 4) + (speed >>> 1)) / speed - 1);
	        UBRR0H= t >>> 8;
	        UBRR0L= t;
	        UCSR0B |= _BV(RXEN0);
	        UCSR0B |= _BV(TXEN0);
